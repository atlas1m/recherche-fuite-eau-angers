...
IMAGE_LIBRARY = {
    ...
    'service': [
        ...
        ('/assets/images/fuite-degat-assurance.jpg', 'Dossier et mallette de diagnostic devant un dégât des eaux'),
...
IMAGE_BY_SLUG = {
    ...
    'recherche-fuite-assurance-angers': ('/assets/images/fuite-degat-assurance.jpg', 'Dossier assurance après dégât des eaux dans un logement'),
    'rapport-recherche-fuite-assurance-angers': ('/assets/images/fuite-degat-assurance.jpg', 'Rapport de recherche de fuite pour dossier assurance'),
...
def _quote_form():
    return f'''<aside class="quote-box" aria-label="Préparation de demande"><div class="quote-ribbon">Demande rapide</div><p class="required">Informations facultatives - validation sur la page contact</p><form action="/contact/" method="get"><label>Nom<input name="name" autocomplete="name"></label><label>Téléphone<input name="phone" autocomplete="tel"></label><label>Email<input name="email" autocomplete="email"></label><label>Commentaire<textarea name="comment" rows="6"></textarea></label><button class="form-button" type="submit">PRÉPARER LA DEMANDE</button><p class="quote-call"><a href="tel:{PHONE_TEL}">Ou appeler directement : {PHONE_DISPLAY}</a></p></form></aside>'''
...
def _proof_block():
    return f'''<section class="proof"><div class="wrap"><div class="proof-cols"><p><strong>Avis vérifiés uniquement.</strong> Aucun témoignage inventé, aucune note artificielle et aucune fausse preuve sociale ne doivent être publiés.</p><p><strong>Cadre white-hat.</strong> Le site reste un service indépendant d’information et d’orientation, sans fausse adresse locale ni fausse équipe artisan.</p><p><strong>Preuve future.</strong> Les retours pourront être ajoutés seulement s’ils sont réels, vérifiables et rattachés à une mise en relation effective.</p></div></div></section>'''
...
def _contact_page(cfg):
    ...
    thumbs = ''.join(_img('thumbs', index=i, loading='lazy') for i in range(len(IMAGE_LIBRARY['thumbs'])))
    return _nav_html() + f'''<main class="contact-layout"><section class="contact-page wrap"><h1>{html.escape(cfg["h1"])}</h1><div class="contact-main"><article class="contact-copy">{_page_img('contact', class_name='contact-main-img', loading='eager')}<div class="contact-text">{contact_intro}</div></article>{_quote_form()}</div><section class="contact-extra"><div class="contact-text">{contact_more}</div><div class="thumb-row contact-thumbs">{thumbs}</div></section></section></main>''' + _footer()
...
def render_reference(slug, cfg):
    ...
    if slug == '':
        body = _nav_html() + _lead_capture('Fuite d’eau à Angers : préparez l’appel', slug) + _intro(cfg) + _service_rows() + _faq_links(cfg) + _areas() + _contact_strip() + _proof_block() + _footer()
