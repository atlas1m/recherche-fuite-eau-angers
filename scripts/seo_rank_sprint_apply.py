...
def _faq_links(cfg):
    ...
    items = ''.join(f'<li><a href="#{i}"><strong>{html.escape(q)}</strong></a><p>{html.escape(a)}</p></li>' for i, (q, a) in enumerate(faq[:6], 1))
    if not items:
        items = ''.join(f'<li><a href="{href}"><strong>{html.escape(label)}</strong></a><p>Voir la page dédiée pour comprendre le symptôme, les informations à préparer et les limites à confirmer avant toute intervention.</p></li>' for href, label, _ in SERVICE_ROWS[:6])
...
def _proof_block():
    return f'''<section class="proof"><div class="wrap"><h2>Cadre de qualification avant mise en relation</h2><div class="proof-cols"><p>Retours clients : affichage uniquement après collecte et source vérifiable. Tant qu’un retour n’est pas sourcé, il ne sert pas de preuve publique et ne doit pas orienter la décision du visiteur.</p><p>Coordonnées et disponibilité : confirmation nécessaire avant orientation. Le site privilégie une information utile, vérifiable et proportionnée à la situation décrite.</p><p>Tarifs et disponibilité : confirmation nécessaire selon accès, méthode, urgence et contexte assurance. La qualification sert justement à séparer une demande simple, un dossier copropriété, une fuite après compteur ou un dégât actif.</p></div><p><a class="call-now dark" href="tel:{PHONE_TEL}">APPELER</a></p></div></section>'''
