...
def _quote_form():
    return f'''<aside class="quote-box" aria-label="Formulaire de demande"><div class="quote-ribbon">Demande rapide</div><p class="required">* champs indicatifs — formulaire non envoyé automatiquement</p><form><label>Nom *<input name="name" autocomplete="name"></label><label>Téléphone *<input name="phone" autocomplete="tel"></label><label>Email *<input name="email" autocomplete="email"></label><label>Commentaire *<textarea name="comment" rows="6"></textarea></label><a class="form-button" href="tel:{PHONE_TEL}">APPELER POUR QUALIFIER</a></form></aside>'''
...
def render_reference(slug, cfg):
    ...
    if slug == '':
        body = _nav_html() + _lead_capture('Recherche Fuite Eau Angers', slug) + _intro(cfg) + _service_rows() + _faq_links(cfg) + _areas() + _contact_strip() + _proof_block() + _footer()
